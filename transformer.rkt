...
;; Version that returns both model function and flat parameter list
;; Needed for optimizer construction
(provide make-transformer-with-params)

(define (make-transformer-with-params src-vocab tgt-vocab
                          #:d-model [d-model 256]
                          #:num-heads [num-heads 8]
                          #:num-layers [num-layers 4]
                          #:d-ff [d-ff 1024]
  ...
  (define all-params
    (append (parameters src-embed)
            (parameters tgt-embed)
            (parameters encoder)
            (parameters output-proj)))
